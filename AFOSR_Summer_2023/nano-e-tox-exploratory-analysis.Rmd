...
```{r}
material_test_org_toxicity_value_endpoint <- ggplot(nano_raw_toxicity_values_mg_l_common_tox, aes(y = nanomaterial, x = value, color = test_organism_type,
                                                    label = test_endpoint, shape = toxicity_measure)) +
  geom_point() +
  theme_minimal() +
  theme(legend.position="none") +
  xlim(0, 1000) +
  ...
  labs(title = "Toxicity of Nanomaterials (NOEC, LOEC, EC50)",
       y = "Nanomaterial",
       x = "Toxicity Value up to 250 (mg/L)")
ggplotly(material_test_org_toxicity_value_endpoint)
```
...
```{r}
a <- ggplot(nano_raw_toxicity_values_mg_l_common_tox, aes(x = test_organism_type,
                                               y = value,
